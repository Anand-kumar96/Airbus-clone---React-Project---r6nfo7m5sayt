...
const Detail = (props) => {
  const [data , setData] = useState([]);  
   ...
   useEffect(() => {
    async function GetData(){
      const fromSource = props.start.charAt(0).toUpperCase() + props.start.slice(1).toLowerCase()
      const toDestination = props.Last.charAt(0).toUpperCase() + props.Last.slice(1).toLowerCase() 
      await fetch(`https://content.newtonschool.co/v1/pr/63b86a1d735f93791e09cb11/flights?from=${fromSource}&to=${toDestination}`)
 ...
 const {setFlightDetails } = useContext(MediumContext)
  function handleFightDetails(){
    setFlightDetails(data[0])
           ...
           shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
           {
            ...
            (
              ...
              <div className='text-3xl font-medium place-self-center' >{data[0].from}</div>
                  ...
                  <img 
                src="https://media0.giphy.com/media/hVxJUqWXVrCGTDpnVU/giphy.gif? 
                  cid=790b761144290eb0b81c365ba65adaefbd0a829c4da1fd76&rid=giphy.gif" 
                  alt="logo" 
                  width={100}
                  height={50}
                  />
                    ...
                    <div className='flex justify-around items-center py-2'>
                      <div className='flex flex-col justify-center content-around border-r-1 ' >
                        <h2 className='border-b-2 text-base my-1 '>
                          ...
                          {data[0].departure.departureTime}</h2>
                                  
                        <h2 className='border-b-2 text-base my-1 '>
                            <span className='font-lg' >Return Time : </span>
                            {data[0].return.returnTime}</h2>

                        <h2 className='border-b-2 text-base my-1 '>
                              <span className='font-lg' >Airplane Name : </span>
                              {data[0].airlineName}</h2>
                       </div>
                 ...
                 :
                 <div className='text-3xl text-grey-400 flex justify-center mt-3' >No Flight Available</div>
             }
